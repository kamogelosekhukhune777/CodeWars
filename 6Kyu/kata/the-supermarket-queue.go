...
is write a function to calculate the total time required for all the customers to check out!
...
func QueueTime(customers []int, n int) int {
	if n == 1 {
		// If there is only one till, the total time is the sum of customer times
		return sum(customers)
	}

	tills := make([]int, n) // Represents the current time each till finishes processing
