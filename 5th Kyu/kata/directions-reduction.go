...
func DirReduc(arr []string) []string {
	...
	for _, dir := range arr {
		...
		if len(stack) > 0 && areOpposites(stack[len(stack)-1], dir) {
			stack = stack[:len(stack)-1] // Remove the last direction from the stack
		} else {
			stack = append(stack, dir) // Add the current direction to the stack
		}
	}
...
func areOpposites(dir1, dir2 string) bool {
	...
	// Check if dir1 and dir2 are opposites
